...
func numbersBiggerThan(_ numberToCompare: Int, _ array: [Int]) -> [Int]? {
    var biggerNumbers: [Int] = [];
    for number in array {
        if(number > numberToCompare) {
            biggerNumbers.append(number)
        }
    }
    
    return biggerNumbers.count > 1 ?  biggerNumbers : nil;
}

//print(numbersBiggerThan(54, [34,39,100,493,1,5]))    // [ 100, 493 ]
...
func repeatCharacter(_ char: Character, _ numberOfRepeats: Int = 1) -> String {
    var result: String = ""
...
func setCase(text: String?, letterCase: Case = .Uppercase) -> String? {
    if(text == nil) {
        return nil;
    }
    switch letterCase {
...
setCase(text: "Interstellar", letterCase: .Uppercase) // INTERSTELLAR
setCase(text: "Interstellar", letterCase: .Lowercase) // interstellar
...
func calculate(_ numbers: Int...) -> (sum: Int, product: Int) {
    var product: Int = 1
    var sum: Int = 0
...
func getCalculator(_ operatorChar: Character?) -> ((_ numbers: [Int]) -> Int)? {
    switch operatorChar{
    case "+":
        return { (_ numbers: [Int]) -> Int in
